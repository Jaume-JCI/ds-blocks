...
__all__ = ['test_data_converter_functions', 'test_no_converter', 'test_standard_converter', 'test_pandas_converter',
...
def test_standard_converter ():
    ...
    def fit_apply (*X, y=None, result_func=None, dc=None):
        ...
        # apply
        return apply (*X, result_func=result_func, dc=dc)

    def apply (*X, result_func=None, dc=None):
        X = dc.convert_single_tuple (X)
        X = dc.convert_before_transforming (*X)
    ...
    dc = StandardConverter (transform_uses_labels=True)
    result = fit_apply (10,  y=1, result_func=transform_returns_labels_singleX, dc=dc)
    assert result==(10,1)
    ...
    dc = StandardConverter (transform_uses_labels=True, labels_returned_by_transform=False)
    ...
    dc = StandardConverter (no_labels=True)
    result = fit_apply (10, result_func=transform_does_not_return_labels_singleX, dc=dc)
    assert result==10
    result = fit_apply (10, 20, result_func=transform_does_not_return_labels_twoXs, dc=dc)
    assert result == (10, 20)
